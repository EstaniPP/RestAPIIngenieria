...
CREATE TABLE IF NOT EXISTS `mydb`.`Email_Reports` (
  ...
  INDEX `fk_Email_Reports_Emergency_Contacts1_idx` (`emergency_contact_id` ASC) VISIBLE,
  CONSTRAINT `fk_Email_Reports_Device_Users`
    FOREIGN KEY (`device_user_id`)
    REFERENCES `mydb`.`Device_Users` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Email_Reports_Emergency_Contacts`
...
CREATE TABLE IF NOT EXISTS `mydb`.`Medical_Personnel` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL,
  `medical_speciality_id` INT NULL,
  `available` BIT NULL,
...
-- -----------------------------------------------------
-- Table `mydb`.`Medical_Personnel_Languages`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Medical_Personnel_Languages` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `language_id` INT NOT NULL,
  `medical_personnel_id` INT NOT NULL,
  INDEX `fk_Languages_has_Medical_Personnel_Medical_Personnel1_idx` (`medical_personnel_id` ASC) VISIBLE,
  INDEX `fk_Languages_has_Medical_Personnel_Languages1_idx` (`language_id` ASC) VISIBLE,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  CONSTRAINT `fk_Medical_Personnel_Languages_Languages`
    FOREIGN KEY (`language_id`)
    REFERENCES `mydb`.`Languages` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Medical_Personnel_Languages_Medical_Personnel`
...
CREATE TABLE IF NOT EXISTS `mydb`.`Patients` (
  ...
  `medical_personnel_id` INT NOT NULL,
  `device_user_id` INT NOT NULL,
  INDEX `fk_Medical_Personnel_has_Device_Users_Device_Users1_idx` (`device_user_id` ASC) VISIBLE,
  INDEX `fk_Medical_Personnel_has_Device_Users_Medical_Personnel1_idx` (`medical_personnel_id` ASC) VISIBLE,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  CONSTRAINT `fk_Patients_Medical_Personnel`
    FOREIGN KEY (`medical_personnel_id`)
    REFERENCES `mydb`.`Medical_Personnel` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Patients_Device_Users`
    FOREIGN KEY (`device_user_id`)
    REFERENCES `mydb`.`Device_Users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `mydb`.`Workout_Exercises` (
  ...
  CONSTRAINT `fk_Workout_Exercises_Exercises`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Workout_Exercises_Workouts`
